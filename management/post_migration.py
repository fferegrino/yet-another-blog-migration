...
POST_IMAGE_REGEX = re.compile(
    r"\{%\s+post_image\s+([a-zA-Z0-9-\._]+)\s+([\w\.\"\s\-#&,]+)\s+%\}"
)
        ...
        with open(full_path, "r") as r:
            ...
            if lines[0] == "---":
                ...
                for line in lines[i + 1 :]:
                    ...
                    if clean_line.startswith("{% highlight"):
                    ...
                    elif clean_line.startswith("{% console ") or clean_line.startswith(
                        "{% endconsole "
                    ):
                        body.append("```  ")
                        ...
                        if re.search(POST_IMAGE_REGEX, clean_line):
                            ...
                            l = re.sub(
                                POST_IMAGE_REGEX,
                                f'<img src="/images/{parts}__' + r'\1" title="\2" />',
                                clean_line,
                            )
                            body.append(l)
                            ...
                            l = re.sub(
                                POST_IMAGE_REGEX_NO_TITLE,
                                f'<img src="/images/{parts}__' + r'\1" />',
                                clean_line,
                            )
                            body.append(l)
                        else:
                            clean_line = line.replace(
                                "site.twitter_username", "MY_TWITTER_HANDLE"
                            )
